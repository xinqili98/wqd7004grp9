...
Airline satisfaction is a crucial aspect of the aviation industry as it directly impacts the loyalty and repeat business of customers. In this report, we will analyze a dataset of Airline satisfaction data collected from Kaggle. We will explore the data and conduct various analysis to understand the behavior of the passengers and their level of satisfaction. The insights and conclusion drawn from this analysis will be useful for the airline industry to improve services and increase passenger satisfaction.
...
2. To understand the correlation between the services provided by Airlines.
3. To understatnd the accuracy of classification model.

# Methodology
## Data Understanding
In this report, we analyze the Airline satisfaction dataset which contains 24 distinct features and 103,904 entries. The dataset includes information on various aspects of the flight experience, including inflight wifi service, ease of online booking, gate location, food and drink, seat comfort, inflight entertainment, on-board service, legroom, baggage handling, check-in service, cleanliness, and passenger demographics such as gender, age, and type of travel. 
The dataset also includes information on departure and arrival delay times, as well as the passenger's overall satisfaction with their flight experience. 
Besides, the data is composed of categorical, ordinal and character variables, which allows us to understand the behavior of the passengers and their level of satisfaction. It will be explored and cleaned to identify any potential issues or outliers before conducting the analysis.
...
* The data provided gives an overview of the customer characteristics of the passengers. 
* Firstly, it can be observed that 18% of the customers are disloyal and 82% are loyal. This indicates that most of the customers have a history of repeat business with the company and can inform strategies to retain and attract loyal customers.
...
* The square root of the total number of observations is commonly used to determine the optimal k value as it balances the trade-off between overfitting and underfitting. Overfitting occurs when a model is too complex and is fitting the noise in the training data, while underfitting occurs when a model is too simple and is not fitting the training data well. And it shows approximate 288.31406486677.

* The 286 & 287 is chosen because of checking the square root of the total number of observations in the training set, as a commonly used rule to determine the optimal k value, which balances the trade-off between overfitting and underfitting.
...
* The result shows that the accuracy of the KNN model is 90.43792 for k = 286 and 90.44755 for k = 287. The accuracy is quite high, which means that the model is performing well and making correct predictions for a high percentage of the test data. Generally, an accuracy of 90% or higher is considered to be very good.

* The confusion matrix results indicate that the KNN model with k = 286 and k = 287 have a high accuracy of around 90%, and good sensitivity and specificity. The model is able to correctly predict the majority of the test data points. The model may have slightly better performance with k = 287, but the difference is very small.
...
# Results
Which model works the best? Justify. (Accuracy, AUC-ROC, F1 score)
Decision Tree - 94.2 %
