...
packages <- c("stringr", "data.table", "ggpubr", "tidyr", "ggrepel", 
              ...
              "caTools", "e1071", "class","party", "htmltools")
...
p_6 <- ggplot(airline_df,aes(x = Flight.Distance,fill = Type.of.Travel)) + geom_density(alpha = 0.5) + labs(title = "Flight Distance by Type of Travel",
  ...
  theme(legend.title = element_text(size = 5))+ theme(legend.key.height= unit(0.5, 'cm'), egend.key.width= unit(0.5, 'cm')) 

# ggarrange(p_1, p_2, p_3, p_4, nrow = 2, ncol=2)  
# ggarrange(p_5, p_6, nrow = 2, ncol=1) 
```
</p>
<li>From the plots, it does appear that most passengers are traveling for business rather than pleasure.</li>
<li>The satisfaction level and loyalty for business travelers is higher than for personal travelers. </li>
<li>The distribution of gender, customer type, and age also varies slightly between the two types of travel, with business travelers tending to be older and have longer flight distances. <li>
</p>


<font size="3">**Satisfaction on the Airline Services from the perspective of "Type of Travel"**</font>  
